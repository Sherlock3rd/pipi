...
仓库：[Sherlock3rd/pipi](https://github.com/Sherlock3rd/pipi)。源码仓库不包含可执行文件，首次使用请按下文构建。构建完成后双击 `dist/Chenpi/Chenpi.exe`。默认在桌面右下角，普通窗口会覆盖它；显示桌面即可看到。右键猫窝可打开设置；猫和其他物品右键暂不响应，不显示常驻设置按钮。行为工作台从设置窗口进入。
