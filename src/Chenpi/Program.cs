    ...
    private void OnLoaded(object sender,RoutedEventArgs e)
    {
        ...
        if(Preview&&Program.Option(args,"--preview-film") is string filmDirectory)
        {
            ...
            filmTimer.Tick+=(_,_)=>{
                ...
                File.WriteAllText(file+".json",System.Text.Json.JsonSerializer.Serialize(new{engine.Now,engine.Action,scene.DisplayedClip,scene.DisplayedFrame,scene.DisplayedDefinition,scene.DisplayedSupportHeight,engine.Support.Kind,engine.State.X,engine.State.Y,engine.State.LitterCover}));
            };filmTimer.Start();
        }
