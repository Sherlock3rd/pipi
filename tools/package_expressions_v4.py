...
def main():
    ...
    manifest={'status':'endpoint-candidates-awaiting-user-review-and-external-video','runtimeChanged':False,'generator':'builtin image_gen','anchors':anchor_records,'clips':rows,'clickWindow':{'seconds':15,'starts':'first qualifying click','reset':'fixed window expiry or leave flat-rest','counts':{'1':'same-pose mild twitch','2':'same-pose stronger twitch then one look-back meow','3':'stand to SR'}}}
    (OUT/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    s=io.StringIO();w=csv.writer(s);w.writerow(['编号','动作','首帧','尾帧','类型','目标秒数','分组']);w.writerows([r['id'],r['name'],r['start'],r['end'],r['type'],r['seconds'],r['group']] for r in rows);(OUT/'动作表.csv').write_text(s.getvalue(),encoding='utf-8-sig')
